...
#define POS 0
template<uint64_t B>
bool isEqual(const mpv::List<uint64_t>& a,const mpv::List<uint64_t>& b){
	if(a.size()!=b.size())
		return false;
	else{
		for(mpv::List<uint64_t>::iterator i=a.rbegin(),j=b.rbegin();i!=a.end();i--,j--){
			if(*i!=*j)
				return false;
		}
		return true;
...
template<uint64_t B>
bool isBigger(const mpv::List<uint64_t>& a,const mpv::List<uint64_t>& b){
	if(a.size()>b.size())
		return true;
...
template<uint64_t B>
bool isSmaller(const mpv::List<uint64_t>& a,const mpv::List<uint64_t>& b){
	if(a.size()<b.size())
		return true;
	else if(a.size()>b.size())
		return false;
...
template<uint64_t B>
bool isBigger_eq(const mpv::List<uint64_t>& a,const mpv::List<uint64_t>& b){
	if(a.size()>b.size())
		return true;
	else if(a.size()<b.size())
		return false;
...
template<uint64_t B>
bool isSmaller_eq(const mpv::List<uint64_t>& a,const mpv::List<uint64_t>& b){
	if(a.size()<b.size())
		return true;
	else if(a.size()>b.size())
		return false;
...
template<uint64_t B>
mpv::List<uint64_t> add(const mpv::List<uint64_t>& a,const mpv::List<uint64_t>& b){
	uint64_t digit;
	bool carry=0;
	mpv::List<uint64_t> res;
	mpv::List<uint64_t>::iterator i=a.begin(),j=b.begin();
...
template<uint64_t B>
mpv::List<uint64_t> sub(const mpv::List<uint64_t>& a,const mpv::List<uint64_t>& b){// a debe ser mayor a b(ni menor ni igual)
	uint64_t digit;
	bool borrow=0;
	mpv::List<uint64_t> res;
	mpv::List<uint64_t>::iterator i=a.begin(),j=b.begin();
	...
	while(res[-1]==0 and res.size()>1)
		res.del(-1);
	return res;
}
template<uint64_t B>
void inc(mpv::List<uint64_t>& a){
	bool carry=1;
	for(mpv::List<uint64_t>::iterator i=a.begin();i!=a.end() and carry==1;i++){
		*i+=carry;
		if(*i/B==0)
			carry=0;
		else
...
template<uint64_t B>
void dec(mpv::List<uint64_t>& a){
	bool borrow=1;
	for(mpv::List<uint64_t>::iterator i=a.begin();i!=a.end() and borrow==1;i++){
		*i-=borrow;
		if(*i>=0){
			borrow=0;
		}
	...
	if(a[-1]==0 and a.size()!=1)
		a.del(-1);
}
template<uint64_t B>
mpv::List<uint64_t> mul(mpv::List<uint64_t> a,mpv::List<uint64_t> b){
	uint64_t digit, carry=0;
	int k=0;
	mpv::List<uint64_t> res=0,aux;
	mpv::List<uint64_t>::iterator i=a.begin(),j=b.begin();
	for(auto i:b){
		aux.clear();
		int K=k;
...
template<uint64_t B>
mpv::List<uint64_t> Kmul(mpv::List<uint64_t> a,mpv::List<uint64_t> c){using namespace std;
	unsigned int size=a.size()>c.size()? a.size():c.size();
	if(size==1){
		mpv::List<uint64_t> res;
		uint64_t digit=(a[0]*c[0])%B;
		uint64_t carry=(a[0]*c[0])/B;//cout<<a[0]<<'*'<<c[0]<<'='<<a[0]*c[0]<<endl;
		if(digit and carry){
			res.append(digit);
			res.append(carry);
		}
		...
		while(c.size()<size)
			c.append(0);
		mpv::List<uint64_t> b=a.cut(0,size/2),d=c.cut(0,size/2);
		mpv::List<uint64_t> ac=Kmul<B>(a,c),bd=Kmul<B>(b,d),ad_bc=sub<B>(sub<B>(Kmul<B>(add<B>(a,b),add<B>(c,d)),bd),ac);
...
/*		while(bd.size()<size)
		...
		mpv::List<uint64_t> res=add<B>(bd,ad_bc);
		while(res[-1]==0 and res.size()>1)
			res.del(-1);
...
//template<uint64_t B>
//mpv::List<uint64_t> div(const mpv::List<uint64_t>& a,const mpv::List<uint64_t>& b){using namespace std;
//	mpv::List<uint64_t> res(0),acum(b);
//	//acum=add<B>(acum,b);
...
template<uint64_t B>
mpv::List<uint64_t> div(const mpv::List<uint64_t>& a,const mpv::List<uint64_t>& b){using namespace std;
	mpv::List<uint64_t> res(0),acum(a);
	//acum=add<B>(acum,b);
	...
	public:
		...
		Int operator^(const Int& other)const{
			if(other.sign==NEG)
				return 0;
			uint64_t cont,acum;
			mpv::List<uint64_t> list(*this);
			for(cont=1;cont<=other;cont*=2){
				list.append(list[-1]*list[-1]);
			}
			Int new_Int=list[-1];
			for(mpv::List<uint64_t>::iterator i=list.rbegin();i!=list.end() and acum!=cont;i--,cont/=2){
				if(acum+cont<=other){
					acum+=cont;
					new_Int*=*i;
				}
...
std::ostream& operator<<(std::ostream& stream,Int<b> n){
	...
	for(mpv::List<uint64_t>::iterator i=x.list.rbegin();i!=x.list.end();i--){
		stream<<*i;
	}
	return stream;
}
