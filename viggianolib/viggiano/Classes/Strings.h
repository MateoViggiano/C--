	...
	class Str{
		...
		public:
			...
			Str operator+(const Str& other)const{
				Str new_string(this->size+other.size);//No se le suma 1 porque el constructor ya tiene en cuenta el espacio extra para el \0 del final
...
/* 				if(new_string.size>=ss_cap){
			...
			void insert(int index,const Str& other){
				if(index<0)
					index+=this->size+1;
				char* this_text=this->get_text();
				int new_size=this->size+other.size;
