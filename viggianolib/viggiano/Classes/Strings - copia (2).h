...
namespace vgg{
	...
	union sbo{
		char stack_str[small_buffer_capacity];
		char* heap_str;
	};
	...
	class Str{
		...
		public:
			char* get_stack(){
				return this->text.stack_str;
			}
			char* get_heap(){
				return this->text.heap_str;
			}
			char* get_text()const{	//devuelve el tipo que se esta usando actualmente en la union. !!! NO se debe modificar el atributo size antes de ser llamada.
				if(this->size>=ss_cap)
					return this->text.heap_str;
				else
					return const_cast<char*>(this->text.stack_str);		
			}
			Str():size(0),text{'\0'}{INCSTRINGS}									//Constructor por defencto
			explicit Str(int sz):size(sz){											//Constructor por tamaño
				INCSTRINGS
				if(sz>=ss_cap)
					this->text.heap_str=new char[sz+1];
				this->get_text()[this->size]='\0';
			}
			Str(const Str& other):size(other.size){									//Constructor de copia
				INCSTRINGS
				if(other.size>=ss_cap){
					this->text.heap_str=new char[size+1];
					strcopy(this->text.heap_str,other.get_text());
				}
				else
					strcopy(this->text.stack_str,other.get_text());
				this->get_text()[this->size]='\0';
			}
			Str(const char* str):size(strsize(str)){								//Constructor con parametors
				...
				if(strsize(str)>=ss_cap){
					this->text.heap_str=new char[size+1];
					strcopy(this->text.heap_str,str);
				}
				else
					strcopy(this->text.stack_str,str);
				this->get_text()[this->size]='\0';
			}
			Str(Str&& other):size(other.size){										//Constructor de movimiento
				INCSTRINGS
				if(other.size>=ss_cap){
					this->text.heap_str=other.get_text();
					this->text.heap_str[this->size]='\0';
				}
				else{
					strcopy(this->text.stack_str,other.get_text());
					this->text.stack_str[this->size]='\0';
				}
				other.text.heap_str=nullptr;
				other.size=0;
			}
			void operator=(const Str& other){										//Operador de copia
				if(this!=&other){
					if(this->size>=ss_cap)
						delete[] this->text.heap_str;
					this->size=other.size;
					if(other.size>=ss_cap){
						this->text.heap_str=new char[other.size+1];
						strcopy(this->text.heap_str,other.get_text());
						this->text.heap_str[this->size]='\0';				
					}
					else{
			...
			void operator=(const char* str){										//Operador de asignacion
				...
				if(this->size>=ss_cap){
					this->text.heap_str=new char[this->size+1];			
					strcopy(this->text.heap_str,str);
					this->text.heap_str[this->size]='\0';
				}
				else{
			...
			void operator=(Str&& other){											//Operador de movimiento	
				...
				this->size=other.size;
				if(other.size>=ss_cap){
					this->text.heap_str=other.text.heap_str;
					this->text.heap_str[this->size]='\0';
				}
				else{
					strcopy(this->text.stack_str,other.get_text());
...
/* 				if(new_string.size>=ss_cap){
					strcopy(new_string.text.heap_str,this->get_text());
					strcopy(new_string.text.heap_str+this->size,other.get_text());//igual que: strcopy(&new_string.text.heap_str[this->size],other.get_text());
				}
				else{
					strcopy(new_string.text.stack_str,this->get_text());
					strcopy(new_string.text.stack_str+this->size,other.get_text());
				} */
				return new_string;
			}
...
/* 				if(new_string.size>=ss_cap){
					strcopy(new_string.text.heap_str,this->get_text());
					strcopy(new_string.text.heap_str+this->size,str);//igual que: strcopy(&new_string.text.heap_str[this->size],other.get_text());
				}
				else{
					strcopy(new_string.text.stack_str,this->get_text());
					strcopy(new_string.text.stack_str+this->size,str);
				} */
				return new_string;
			}
			void operator+=(const Str& other){
				...
				if(new_size>=ss_cap){
					char* new_text=new char[new_size+1];//puntero auxiliar porque si escribo sobre this->text->heap_str, al ser una union, puedo estar sobreescribiendo aux ya que este podria ser this->text->stack_str.
					strcopy(new_text,this_text);
					strcopy(new_text+this->size,other.get_text());
					this->text.heap_str=new_text;
					this->text.heap_str[new_size]='\0';
				}
				else{
					strcopy(this->text.stack_str+this->size,other.get_text());
					this->text.stack_str[new_size]='\0';
				}
				if(this->size>=ss_cap)
					delete[] this_text;
				this->size=new_size;		
			...
			void operator+=(const char* str){
				...
				if(new_size>=ss_cap){
					...
					strcopy(this->text.stack_str+this->size,str);
					this->text.stack_str[new_size]='\0';
				}
				if(this->size>=ss_cap)
					delete[] this_text;
			...
			void operator+=(char chr){
				...
				if(new_size>=ss_cap){
					...
					this->text.stack_str[this->size]=chr;
					this->text.stack_str[new_size]='\0';
				}
				if(this->size>=ss_cap)
					delete[] this_text;
				...
				if(num){
					...
					if(new_size>=ss_cap){
						...
						for(int index=this->size;index<new_size;index+=this->size)
							memcopy(this->text.stack_str+index,this_text,this->size);
						this->text.stack_str[new_size]='\0';
					}
					if(this->size>=ss_cap)
						delete[] this_text;
			...
			void insert(unsigned int index,const Str& other){
				...
				if(new_size>=ss_cap){
					...
					char* insert_position=this->text.stack_str+index;
			...
			void erase(int start,int end){
				...
				if(new_size>=ss_cap){
					...
					if(this->size>=ss_cap)
						memcopy(this->text.stack_str,this_text,start);
					strcopy(this->text.stack_str+start,this_text+end);
					this->text.stack_str[new_size]='\0';
				}
				if(this->size>=ss_cap)
					delete[] this_text;
		 	...
		 	void replace(const Str& substring1,const Str& substring2,int max_count=-1){
				...
				if(new_size>=ss_cap){
					...
					for(int i=array[0],j=array[0],k=0;i<new_size;){
						if(i==array[k]){
							strcopy(this->text.stack_str+array[k++],substring2_text);
							i+=substring2.size;
							j+=substring1.size;
						}
						else{
							this->text.stack_str[i]=aux[j];
							i++;
							j++;
						}
					}
			...
			Str extract(int start,int end=-1)const{
				if(end<0)
					end=this->size;
			...
			Str substr(int index,int pos=-1)const{
				if(pos<0)
					pos=this->size-index;
			...
			void noExtraSpaces(){
				...
				if(new_size>=ss_cap){
					...
					for(int i=0,j=0;i<this->size;i++){
						if(not(this_text[i]==' ' and this_text[i+1]==' ')){
							this->text.stack_str[j]=this_text[i];
							j++;
						}
					}
					this->text.stack_str[new_size]='\0';		
 			...
 			void lstrip(){
				...
				if(new_size>=ss_cap){
					...
					for(int i=0;count<=this->size;i++,count++)
						this->text.stack_str[i]=this_text[count];
				if(this->size>=ss_cap)
					delete[] this_text;
				this->size=new_size;
			}
			...
			void upper(){
				if(this->size>=ss_cap)
					for(int i=0;i<size;i++)
						if(this->text.heap_str[i]>=97 and this->text.heap_str[i]<=122)
							this->text.heap_str[i]-=32;
				else
					for(int i=0;i<size;i++)
						if(this->text.stack_str[i]>=97 and this->text.stack_str[i]<=122)
							this->text.stack_str[i]-=32;
			}
			void lower(){
				if(this->size>=ss_cap)
					for(int i=0;i<size;i++)
						if(this->text.heap_str[i]>=65 and this->text.heap_str[i]<=90)
							this->text.heap_str[i]+=32;
				else
					for(int i=0;i<size;i++)
						if(this->text.stack_str[i]>=65 and this->text.stack_str[i]<=90)
							this->text.stack_str[i]+=32;
			}
			...
			Str upper_cpy()const{
				...
				if(this->size>=ss_cap)
					for(int i=0;i<size;i++)
						new_string[i]=this->text.heap_str[i]>=97 and this->text.heap_str[i]<=122 ? this->text.heap_str[i]-32:this->text.heap_str[i];
				else
					for(int i=0;i<size;i++)
						new_string[i]=this->text.stack_str[i]>=97 and this->text.stack_str[i]<=122 ? this->text.stack_str[i]-32:this->text.stack_str[i];
				return new_string;
			}
			Str lower_cpy()const{
				Str new_string(this->size);
				if(this->size>=ss_cap)
					for(int i=0;i<size;i++)
						new_string[i]=this->text.heap_str[i]>=65 and this->text.heap_str[i]<=90 ? this->text.heap_str[i]+32:this->text.heap_str[i];
				else
					for(int i=0;i<size;i++)
						new_string[i]=this->text.stack_str[i]>=65 and this->text.stack_str[i]<=90 ? this->text.stack_str[i]+32:this->text.stack_str[i];
				return new_string;
			}
	...
	template<short cap>
	void getstr(Str<cap>& string,const int reader_size=500){
		char reader[reader_size+1];
		bool end_loop=false;
		while(not end_loop){
			fgets(reader,reader_size+1,stdin);
